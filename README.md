...
and is independent of the JDK running the tool (runtime 11 can parse
sources 8).
...
* JDK 11 (`java-11-openjdk`), compiled with `javac --release 11` (or any JDK with `--release 11` support, e.g., 17/21/26)
...
`--java` selects the Java parser (default 8). Runtime is always 11.
