...
public enum OutputFormat {
    ...
    public static OutputFormat from(String raw) {
        ...
        if ("json".equals(v)) return JSON;
        if ("xml".equals(v)) return XML;
        if ("yaml".equals(v) || "yml".equals(v)) return YAML;
        if ("toon".equals(v)) return TOON;
        throw new IllegalArgumentException("unsupported output format: " + raw + " (use json, xml, yaml, toon)");
    }
