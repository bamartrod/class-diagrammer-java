...
final class SignatureInterpreter {
    ...
    SignatureInterpreter(String simpleName, TypeKind kind, boolean isEnum) { this(simpleName, kind, isEnum, java.util.Collections.emptyList()); }

    SignatureInterpreter(String simpleName, TypeKind kind, boolean isEnum, List<String> classImports) {
        this.simpleName = simpleName;
        this.kind = kind;
        this.awaitingEnumConstants = isEnum;
        this.classImports = classImports == null ? java.util.Collections.emptyList() : java.util.Collections.unmodifiableList(new ArrayList<String>(classImports));
    }
    ...
    void accept(String chunk) {
        ...
        if (signatureSide.isEmpty()) return;
        int parenIndex = signatureSide.indexOf('(');
        if (parenIndex < 0) interpretWithoutParentheses(signatureSide);
        else interpretWithParentheses(signatureSide, parenIndex);
    }

    SignatureInterpreter.ParsedMembers toMembers() { return new ParsedMembers(methods, constructors, fields); }

    private void interpretWithoutParentheses(String side) {
        List<String> tokens = stripModifiers(TokenOps.splitWords(side));
        if (awaitingEnumConstants && tokens.size() == 1 && TokenOps.isIdentifier(tokens.get(0))) return;
        if (tokens.size() == 1 && TokenOps.isModifier(tokens.get(0))) return;
        if (tokens.size() < 2) return;
        String name = tokens.get(tokens.size() - 1);
        if (!TokenOps.isIdentifier(name)) return;
        String type = join(tokens, tokens.size() - 1);
        List<String> required = ImportResolver.requiredImports(type, classImports);
        fields.add(Field.named(name, type, effectiveVisibility(), currentModifiers, required));
    ...
    private void interpretWithParentheses(String side, int parenIndex) {
        ...
        if (tokens.size() == 1) {
            ...
            if (candidate.equals(simpleName)) {
                ...
                for (Parameter p : parameters) paramTypes.add(p.type());
                List<String> required = ImportResolver.requiredImportsForMethod("", paramTypes, classImports);
                constructors.add(Method.constructor(candidate, effectiveVisibility(), currentModifiers, parameters, required));
                awaitingEnumConstants = false;
                return;
            }
            if (!awaitingEnumConstants || !TokenOps.isIdentifier(candidate)) { /* discard */ }
            return;
        }
        String name = tokens.get(tokens.size() - 1);
        if (!TokenOps.isIdentifier(name)) return;
        ...
        List<String> paramTypes = new ArrayList<>();
        for (Parameter p : parameters) paramTypes.add(p.type());
        List<String> required = ImportResolver.requiredImportsForMethod(returnType, paramTypes, classImports);
        methods.add(Method.returning(name, returnType, effectiveVisibility(), currentModifiers, parameters, required));
        awaitingEnumConstants = false;
    }
    ...
    private List<Parameter> parseParameters(String body) {
        ...
        for (String raw : TokenOps.splitTopLevel(body, ',')) {
            ...
            if (tokens.size() == 1) { parameters.add(new Parameter(tokens.get(0), "")); continue; }
            String name = tokens.get(tokens.size() - 1);
            String type = join(tokens, tokens.size() - 1);
    ...
    private String join(List<String> tokens, int upToExclusive) { return String.join(" ", tokens.subList(0, upToExclusive)); }

    private List<String> stripModifiers(List<String> tokens) {
        List<String> kept = new ArrayList<>();
        for (String token : tokens) {
            if (kept.isEmpty() && TokenOps.isModifier(token)) captureModifier(token);
            else kept.add(token);
        }
        return kept;
    }
    ...
    private List<String> stripModifiersAndTypeParameters(List<String> tokens) {
        ...
        for (String token : tokens) {
            if (kept.isEmpty() && TokenOps.isModifier(token)) captureModifier(token);
            else if (kept.isEmpty() && token.startsWith("<")) { /* generic */ }
            else kept.add(token);
        }
        return kept;
    }
    ...
    private void captureModifier(String token) {
        if (token.equals("public")) currentVisibility = Visibility.PUBLIC;
        else if (token.equals("private")) currentVisibility = Visibility.PRIVATE;
        else if (token.equals("protected")) currentVisibility = Visibility.PROTECTED;
        currentModifiers.add(token);
    }

    private Visibility effectiveVisibility() {
        if (kind == TypeKind.INTERFACE && currentVisibility == Visibility.PACKAGE_PRIVATE) return Visibility.PUBLIC;
        return currentVisibility;
    }

    private void resetForNextChunk() { currentVisibility = Visibility.PACKAGE_PRIVATE; currentModifiers = new LinkedHashSet<>(); }
    ...
    static final class ParsedMembers {
        ...
        List<Method> constructors() { return constructors; }
