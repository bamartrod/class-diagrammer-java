...
public final class ConformanceResult {
    ...
    public static ConformanceResult aggregate(List<EvaluationResult> results) {
        if (results == null || results.isEmpty()) {
            return new ConformanceResult(List.of(), EvaluationState.CONFORMANT);
        }
        ...
        for (EvaluationResult r : results) {
            ...
            switch (r.state()) {
                case NON_CONFORMANT -> hasNonConformant = true;
                case REVIEW_REQUIRED -> hasReviewRequired = true;
                case UNDECIDABLE -> hasUndecidable = true;
        ...
        List<EvaluationResult> sorted = results.stream().sorted((a,b) -> a.ruleId().compareTo(b.ruleId())).toList();
        return new ConformanceResult(sorted, agg);
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof ConformanceResult that)) return false;
