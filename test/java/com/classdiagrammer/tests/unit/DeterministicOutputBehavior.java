...
public final class DeterministicOutputBehavior {
    ...
    public static void verify(TestHarness h) {
        ...
        h.expect("identical source and config produce identical node ordering", () -> {
            ...
            List<String> ids1 = s1.lastReport.nodes().stream().map(n -> n.qualifiedName()).sorted().collect(java.util.stream.Collectors.toList());
            List<String> ids2 = s2.lastReport.nodes().stream().map(n -> n.qualifiedName()).sorted().collect(java.util.stream.Collectors.toList());
        ...
        h.expect("json output nodes are sorted deterministically", () -> {
            ...
            List<String> order = sink.lastReport.nodes().stream().map(n -> n.qualifiedName()).collect(java.util.stream.Collectors.toList());
            List<String> sorted = order.stream().sorted().collect(java.util.stream.Collectors.toList());
